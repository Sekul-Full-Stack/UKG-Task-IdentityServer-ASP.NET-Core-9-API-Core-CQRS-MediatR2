...
const AllUsers = () => {
    ...
    useEffect(() => {
      const fetchRoles = async () => {
        try {
          ...
          const response = await fetch('http://localhost:5151/api/people/admin/all-roles',{
            method: 'GET',
            headers: {
    ...
    const handleAssignRole = async () => {
        ...
        try {  
            const token = localStorage.getItem('token'); 
            const response = await fetch('http://localhost:5151/api/people/admin/assign-role', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}` 
    ...
    const handleResetPassword = async () => {  
        ...
        try { 
          ...
          const response = await fetch(
            currentUser.id === selectedUser.Id ? 'http://localhost:5151/api/people/me/reset-password' :
