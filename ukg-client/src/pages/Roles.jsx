...
const Roles = () => {
    ...
    const fetchRoles = async () => {
      try {
        ...
        const res = await fetch('http://localhost:5151/api/people/admin/all-roles', {
          method: 'GET',
          headers: {
    ...
    const handleSubmit = async (e) => {
      ...
      const url = editingRoleId
        ? `http://localhost:5151/api/people/update-role/${editingRoleId}`
    ...
    const handleDelete = async (id) => {
      try {
        const token = localStorage.getItem('token');
        const res = await fetch(`http://localhost:5151/api/people/delete-role/${id}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}` 
